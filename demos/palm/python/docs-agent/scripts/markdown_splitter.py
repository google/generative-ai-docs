...
import re
import os
from scripts import tokenCount
import frontmatter


class Section:
    def __init__(
        self, id: int, name_id: str, page_title: str, section_title: str, level: int, previous_id: int, parent_tree: list[int], token_count: float, content: str
    ):
        self.id = id
...
Content: {self.content}\n"
    ...
    def encodeToChromaDBNoContent(self):
        ...
        metadata.update({"parent_tree": self.parent_tree})
        metadata.update({"token_estimate": float(self.token_count)})
        return metadata

    def decodeFromChromaDB(metadata):
        ...
        if "content" in metadata:
            ...
            content = ""
        section = Section(section_id, section_name_id, page_title, section_title, section_level, previous_id, parent_tree, token_estimate, content)
        return section
...
# class dbObjSection:
#     def __init__(self, UUID, md_hash, section):
#         self.UUID = UUID
...
def process_markdown_page(markdown_text):
    page_metadata = {}
