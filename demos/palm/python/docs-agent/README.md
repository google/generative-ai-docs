...
2. Launch the Docs Agent chat app:
   ...
   **Note**: If this ```poetry run ./chatbot/launch.sh``` command fails to run, check the ```HOSTNAME```
   environment variable on your host machine (for example, ```echo $HOSTNAME). If this variable is unset,
